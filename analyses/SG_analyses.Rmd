...
ggplot(lda_scores, aes(x = LD1, y = LD2, color = site_type, shape = site_type)) +
  ...
  labs(
    ...
    subtitle = "95% confidence ellipses by site type",
...
#FIG 3: Tornado Plot of Predominant Coefficients 2024 vs. 2025
...
#getting rid of NAs
merged_clean_data[sapply(merged_clean_data, is.numeric)] <-
  lapply(merged_clean_data[sapply(merged_clean_data, is.numeric)],
         function(x) { x[is.na(x)] <- 0; x })
...
merged_clean_data$site_type <- as.factor(merged_clean_data$site_type)
...
# 3. Rename those columns by removing "20m2_"
names(merged_clean_data)[names(merged_clean_data) %in% cols_to_scale] <-
  gsub("20m2","", cols_to_scale)
...
ggplot(rec_2_s_long, aes(x = reorder(variable, value), y = value_signed, fill = factor(year))) +
  ...
  #just doing density right now because not sure how to standardize with cover data 
...
##Patch 2: REC_04 Deep
```{r}
rec_4_d <- merged_clean_data %>%
  dplyr::filter(site == "REC_04", 
                zone == "Deep", 
                site_type == "BAR") %>%
  group_by(year, site, site_type, zone) %>%
  summarise(across(c(
