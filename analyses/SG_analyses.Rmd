...
#FIG 3: Heatmap of Predicted Patch Transitions
```{r}
library(ggplot2)
library(reshape2)
library(viridis)
...
```{r}
#Comparing BAR patches which stayed BAR to transitioned BAR patches
...
sim_cover <- simper(comm_cover, group, permutations = 999, trace = FALSE)
sim_cover_summary <- summary(sim_cover, ordered = TRUE, digits = 3)
...
# --- 7. Significant environmental vectors ---
...
# --- 9. Plot NMDS with PERMANOVA annotation ---
ggplot() +
  geom_point(
    data = site_scores,
...
# --- 3. Extract site scores ---
site_scores <- as.data.frame(scores(nmds_model, display = "sites"))
site_scores$pred_patch <- incip_for$pred_patch  # group labels

# --- 4. Extract significant environmental vectors ---
vec_scores <- as.data.frame(scores(envfit_model, display = "vectors"))
vec_scores$pval <- envfit_model$vectors$pvals
vec_scores <- vec_scores %>% filter(pval < 0.05)
vec_scores$variable <- rownames(vec_scores)
...
comm_cols <- c(
  ...
  "macr", "macj", "nerj", "ptej",
  "lsetj", "eisj",
  grep("^cov_", names(bar_bar_for), value = TRUE)
)

comm_matrix <- bar_bar_for[, comm_cols]

# Hellinger transform to downweight zeros
comm_hellinger <- decostand(comm_matrix, method = "hellinger")

# --- 2. Prepare environmental variables ---
bar_bar_for$pred_patch <- as.factor(bar_bar_for$pred_patch)

env_vars <- c("risk_index", "relief_cm", "purple_urchin_densitym2","red_urchin_densitym2", "prop_purp_concealedm2", "prop_red_concealedm2",
              "tegula_densitym2", "pomaulax_densitym2", "zone")
env_matrix <- bar_bar_for[, env_vars]
...
# --- 3. Extract site scores ---
site_scores <- as.data.frame(scores(nmds_model, display = "sites"))
site_scores$pred_patch <- bar_bar_for$pred_patch  # group labels
...
ggplot() +
  ...
  geom_point(data = site_scores, aes(x = NMDS1, y = NMDS2, color = pred_patch), size = 3, alpha = 0.8) +
  
  # 95% confidence ellipses for each group
  stat_ellipse(data = site_scores, aes(x = NMDS1, y = NMDS2, color = pred_patch), 
               type = "t", linetype = 2, size = 1, alpha = 0.2, show.legend = FALSE) +
  
  # Environmental arrows
  geom_segment(data = vec_scores, 
               aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2),
               arrow = arrow(length = unit(0.35, "cm")), color = "darkred", size = 1) +
  
  # Arrow labels
  geom_text_repel(data = vec_scores, 
                  aes(x = NMDS1, y = NMDS2, label = variable),
                  color = "darkred", size = 4, fontface = "bold") +
  
  # Theme and labels
  theme_minimal(base_size = 14) +
  labs(title = "NMDS Biplot: Community Composition vs Significant Environmental Drivers",
       x = "NMDS1", y = "NMDS2", color = "Predicted Patch") +
  scale_color_brewer(palette = "Set1") +
  theme(
    legend.position = "right",
...
individual_sp <- bar_incip %>%
  ...
  summarise(across(
    c(macro_stipe_density_, macj, macr, density_nerlue, nerj,
      density_ptecal, ptej, lamr, density_lamset, lsetj,
      purple_urchin_densitym2, prop_purp_concealedm2, cov_articulated_coralline,
  ...
  rename(
    macrocystis_adults = macro_stipe_density_,
    macrocystis_juveniles = macj,
    macrocystis_recruits = macr,
    nereocystis_adults = density_nerlue,
    nereocystis_juveniles = nerj,
    pterygophora_adults = density_ptecal,
    pterygophora_juveniles = ptej,
    laminariales_spp_recruits = lamr,
    lam_setchellii_adults = density_lamset,
    lam_setchellii_juveniles = lsetj,
    total_purple_urchins = purple_urchin_densitym2,
    concealed_purple_urchins = prop_purp_concealedm2,
...
# Define species order for plotting
species_order <- c("total_purple_urchins", "concealed_purple_urchins")
...
cover_species <- c("articulated_coralline", "crustose_coralline", "encrusting_red",
                   "fleshy_reds", "stephanocystis_adult", "desmerestia_spp")
urchins <- c("total_purple_urchins", "concealed_purple_urchins")
...
#facetwrap
```{r}
# Get min and max across both datasets
y_range <- range(c(algal_density_tornado_2$value_signed,
                   algal_density_tornado_3$value_signed),
                 na.rm = TRUE)

y_limits <- c(floor(min(y_range)), ceiling(max(y_range)))


library(ggplot2)

p1 <- ggplot(algal_density_tornado_2, 
       aes(x = reorder(individual_species, abs(mean_value)), 
           y = value_signed, 
           fill = factor(year))) +
  geom_col(width = 0.7, color = "gray20") +
...
p2 <- ggplot(algal_density_tornado_3, 
       aes(x = reorder(individual_species, abs(mean_value)), 
           y = value_signed, 
           fill = factor(year))) +
  geom_col(width = 0.7, color = "gray20") +
