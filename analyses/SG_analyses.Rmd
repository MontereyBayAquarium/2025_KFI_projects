...
summary(comm_hellinger[, comm_cols])
anyNA(comm_hellinger[, comm_cols])
cor(comm_hellinger[, comm_cols])
...
# nmds
nmds_model <- metaMDS(comm_hellinger, distance = "bray", k = 2, trymax = 300)
...
# --- FIX: match site_scores and pred_patch correctly ---
site_scores <- as.data.frame(scores(nmds_model, display = "sites"))
site_scores$site_id <- rownames(site_scores)            # keep site IDs
bar_for$site_id <- rownames(bar_for)                    # add site IDs to bar_for

site_scores <- left_join(
  site_scores,
  bar_for[, c("site_id", "pred_patch")], 
  by = "site_id"
)

# Check grouping counts after join
print(table(site_scores$pred_patch))

# --- Get significant environmental vectors (unchanged) ---
vec_scores <- as.data.frame(scores(envfit_model, display = "vectors"))
vec_scores$pval <- envfit_model$vectors$pvals
vec_scores <- vec_scores %>% filter(pval < 0.05)
vec_scores$variable <- rownames(vec_scores)
...
vec_scores <- vec_scores %>%
  mutate(
    NMDS1 = NMDS1 * envfit_model$vectors$r[rownames(vec_scores)],
    NMDS2 = NMDS2 * envfit_model$vectors$r[rownames(vec_scores)]
  )
...
ggplot() +
  ...
  # Label faraway points so they are not "hidden"
  geom_text(
    data = faraway_points,
    aes(x = NMDS1, y = NMDS2, label = site_id),
    vjust = -0.8, hjust = 0.5,
    size = 3.5,
    color = "black"
  ) +
  # Color scale
  scale_color_manual(values = c("BAR" = "red", "FOR" = "blue")) +
  # Axis limits auto-scaled to include all points
  coord_cartesian(expand = TRUE) +
  theme_bw() +
  theme(
    panel.grid.major = element_blank(),
...
rowSums(comm_matrix)  # sums per site
...
# --- Filter dataset for transitions to forest ---
three_groups <- patch_trans_data %>%
  filter(str_detect(patch_transition, "BAR.*→ FOR|INCIP.*→ FOR|FOR.*→ FOR")) %>%
  filter(year == "2025") %>%
  mutate(
    forest_type = case_when(
      str_detect(patch_transition, "FOR.*→ FOR") ~ "Persistent",
      str_detect(patch_transition, "BAR.*→ FOR") ~ "BAR_New",
      str_detect(patch_transition, "INCIP.*→ FOR") ~ "INCIP_New"
    )
  )
...
comm_matrix <- three_groups[, comm_cols]

# Give the community matrix row names to match metadata
rownames(comm_matrix) <- rownames(three_groups)

# --- Remove rows with all zeros ---
comm_matrix <- comm_matrix[rowSums(comm_matrix, na.rm = TRUE) > 0, ]

# --- Hellinger transform ---
comm_hellinger_3 <- decostand(comm_matrix, method = "hellinger")

# --- Run NMDS ---
set.seed(42)
nmds3 <- metaMDS(comm_hellinger_3, distance = "bray", k = 2, trymax = 100)

# Remove species with no variation
comm_envfit <- comm_hellinger_3[, apply(comm_hellinger_3, 2, sd) > 0]

# Fit species vectors
set.seed(42)
fit_species <- envfit(nmds3, comm_envfit, permutations = 999)

# Extract vector scores
fit_scores <- as.data.frame(scores(fit_species, "vectors"))
fit_scores$species <- rownames(fit_scores)
fit_scores$r2 <- fit_species$vectors$r
fit_scores$pval <- fit_species$vectors$pvals

# Keep only significant species (p < 0.05)
sig_species <- fit_scores %>%
  filter(pval <= 0.05) %>%
  arrange(desc(r2)) %>%
  slice_head(n = 10)  # top 10 by r² strength

arrow_multiplier <- ordiArrowMul(fit_species)
...
# --- Extract site scores ---
nmds3_scores <- as.data.frame(scores(nmds3, display = "sites"))

# Merge NMDS scores with metadata using row names
nmds3_scores <- nmds3_scores %>%
  mutate(row_id = rownames(.)) %>%
  left_join(
    three_groups %>%
      mutate(row_id = rownames(.)) %>%
      select(row_id, forest_type),
    by = "row_id"
  )

# --- Plot ---
ggplot(nmds3_scores, aes(x = NMDS1, y = NMDS2, color = forest_type)) +
  ...
  theme_minimal(base_size = 14) +
  labs(
    title = "NMDS of Forest Communities",
    subtitle = "Top 10 significant species vectors (p < 0.05)",
    color = "Forest Type"
  )
print(three_groups)
#PERMANOVA
permanova_res <- adonis2(comm_hellinger_3 ~ patch_transition, data = three_groups, permutations = 999)
R2 <- round(permanova_res$R2[1], 3)
pval <- permanova_res$`Pr(>F)`[1]
#no significant difference between forest communities of recovered forest patches to persistent forests 
```
```{r}
permanova_res <- adonis2(comm_hellinger_3 ~ forest_type, data = three_groups, permutations = 999)
R2 <- round(permanova_res$R2[1], 3)
pval <- permanova_res$`Pr(>F)`[1]
