...
#community (species density and cover) data
comm <- bar_incip_for[, c(5, 8, 10:11, 13:14, 21, 23:26, 28:37, 39:50)]


# Remove rows that are all zeros
comm_clean <- comm[rowSums(comm != 0) > 0, ]

#turn into community matrix 
m_comm = as.matrix(comm_clean)

# Run NMDS
set.seed(123)
nmds <- metaMDS(comm_clean, distance = "bray", autotransform = TRUE)
nmds
...
points(nmds, display="sites", col=pred_patch)


#environmental data (herbivores, reef complexity)
env <- bar_incip_for %>%
  group_by(site, zone, year, pred_patch) %>%
  dplyr::select(7, 15:20, 28:29)
