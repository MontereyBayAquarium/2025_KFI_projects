...
load_or_download <- function(local_path, sheet_url, sheet_number) {
  if (file.exists(local_path)) {
    message(paste("✅ Loading", basename(local_path), "from local .rds..."))
    readRDS(local_path)
  } else {
    message(paste("📥 Downloading", basename(local_path), "from Google Sheets..."))
    df <- read_sheet(sheet_url, sheet = sheet_number) %>% clean_names()
    saveRDS(df, local_path)
    message(paste("💾 Saved", basename(local_path), "locally."))
    df
  }
}
