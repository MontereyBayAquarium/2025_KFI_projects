...
#D)BAR25 -> INCIP25, FOR25
```{r}
#Comparing BAR patches which stayed BAR to transitioned BAR patches

bar_incip_for <- patch_trans_data %>%
  filter(str_detect(patch_transition, "BAR.*→ (BAR|INCIP|FOR)")) %>%
  filter(year == "2025") %>%
...
sim_density <- simper(comm_density, group, permutations = 999, trace = FALSE)
sim_density_summary <- summary(sim_density, ordered = TRUE, digits = 3)
...
comm_matrix <- bar_incip_for[, comm_cols]
...
bar_incip_for$zone <- as.factor(bar_incip_for$zone)
bar_incip_for$pred_patch <- as.factor(bar_incip_for$pred_patch)

env_vars <- c("risk_index", "relief_cm", "purple_urchin_densitym2","red_urchin_densitym2", "purple_urchin_conceiledm2",
              "tegula_densitym2", "pomaulax_densitym2", "zone")
env_matrix <- bar_incip_for[, env_vars]

# --- 3. Run RDA ---
rda_model <- rda(comm_hellinger ~ risk_index + relief_cm + purple_urchin_densitym2 + red_urchin_densitym2+
                 purple_urchin_conceiledm2 + tegula_densitym2 + pomaulax_densitym2 + zone,
                 data = env_matrix)

# --- 4. Extract scores for plotting ---
# Sample sites
site_scores <- as.data.frame(scores(rda_model, display="sites", scaling=2))
site_scores$sample <- rownames(site_scores)
site_scores$pred_patch <- bar_incip_for$pred_patch
...
ggplot() +
  # Sites colored by zone
  geom_point(data=site_scores, aes(x=RDA1, y=RDA2, color=pred_patch), size=3) +
  # Species names
  geom_text_repel(data=species_scores, aes(x=RDA1, y=RDA2, label=species),
                  color="black", size=3) +
  # Environmental arrows
  geom_segment(data=env_scores, aes(x=0, y=0, xend=RDA1, yend=RDA2),
               arrow=arrow(length=unit(0.3,"cm")), color="red") +
  geom_text_repel(data=env_scores, aes(x=RDA1, y=RDA2, label=variable),
                  color="red", size=3) +
  theme_minimal() +
  labs(title="RDA Biplot: Community composition vs Environmental variables",
       x="RDA1", y="RDA2") +
  scale_color_brewer(palette="Set1") +
